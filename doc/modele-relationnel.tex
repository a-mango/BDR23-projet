...
\begin{itemize}
    \item \ttt{person (\ul{id}, phone\_no, name, comment)}
    \subitem \ttt{name} \ttt{NOT NULL}
    \subitem \ttt{phone\_no} \ttt{UNIQUE}

    \item \ttt{customer (\ul{id}, tos\_accepted, private\_note)}
    \subitem \ttt{id} référence \ttt{person.id}
    \subitem \ttt{tos\_accepted} \ttt{NOT NULL}

    \item \ttt{collaborator (\ul{id}, email)}
    \subitem \ttt{id} référence \ttt{Person.id}
    \subitem \ttt{email} \ttt{NOT NULL}

    \item \ttt{manager (\ul{id})}
    \subitem \ttt{id} référence \ttt{collaborator.id} 

    \item \ttt{technician (\ul{id})}
    \subitem \ttt{id} référence \ttt{collaborator.id}

    \item \ttt{receptionist (\ul{id})}
    \subitem \ttt{id} référence \ttt{collaborator.id}

    \item \ttt{language (\ul{name})}

    \item \ttt{receptionist\_language (\ul{id}, \ul{language\_id})}
    \subitem \ttt{id} référence \ttt{receptionist.id}
    \subitem \ttt{language\_id} référence \ttt{language.name}

    \item \ttt{specialization (\ul{name})}
    
    \item \ttt{technician\_specialization (\ul{id}, \ul{spec\_id})}
    \subitem \ttt{id} référence \ttt{technician.id}
    ...
    \subitem \ttt{receptionist\_id} référence \ttt{receptionist.id} \ttt{NOT NULL}
